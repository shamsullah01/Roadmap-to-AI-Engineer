...
import { LEVELS, TOPIC_XP, TOTAL_TOPICS, TOPIC_KEYS } from "./data";
...
export function useProgress() {
  ...
  // A topic is unlocked if it's the first in the sequence or the one before it is done.
  const isUnlocked = (id) => {
    const i = TOPIC_KEYS.indexOf(id);
    if (i <= 0) return true;
    return !!completed[TOPIC_KEYS[i - 1]];
  };

  const toggleTopic = (id) =>
    setCompleted((c) => {
      const i = TOPIC_KEYS.indexOf(id);
      if (i < 0) return { ...c, [id]: !c[id] };

      // Can't check a topic until its predecessor is complete.
      if (i > 0 && !c[TOPIC_KEYS[i - 1]]) return c;

      if (!c[id]) return { ...c, [id]: true };

      // Unchecking a topic re-locks everything after it, so clear the tail too.
      const next = { ...c };
      for (let j = i; j < TOPIC_KEYS.length; j++) next[TOPIC_KEYS[j]] = false;
      return next;
    });
  ...
  return { completed, toggleTopic, isUnlocked, addXp, reset, hydrated, syncState, ...derived };
}
